...
export class PatientService {
  ...
  create(patient: PatientRequest): Observable<PatientResponse> {
    return this.http.post<PatientResponse>(`${this.config.apiUrl}/api/patients/create`, patient);
  }

  delete(id: string): Observable<void> {
    return this.http.delete<void>(`${this.config.apiUrl}/api/patients/delete/${id}`);
  }

  update(id: string, patient: Omit<PatientRequest, "registeredDate">): Observable<PatientResponse> {
    return this.http.put<PatientResponse>(`${this.config.apiUrl}/api/patients/update/${id}`, patient);
  }

  getById(id: string): Observable<PatientResponse> {
    return this.http.get<PatientResponse>(`${this.config.apiUrl}/api/patients/${id}`);
  }
  ...
  startImport(s3Key: string, mapping: Record<string, string>, totalRows: number): Observable<ImportStartResponse> {
    return this.http.post<ImportStartResponse>(`${this.config.apiUrl}/api/patients/import/start`, { s3Key, mapping, totalRows });
  }

  getImportJobStatus(jobId: string): Observable<ImportJobStatus> {
    return this.http.get<ImportJobStatus>(`${this.config.apiUrl}/api/patients/import/status/${jobId}`);
  }
}
